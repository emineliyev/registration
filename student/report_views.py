...
#             overdue_payments = student.tuitionfeesplit_set.filter(payment_date__isnull=True, installment_date__lt=today)
#             student.is_debtor = overdue_payments.exists()
#             student.save()
#
...
#         self.update_debtor_statuses()  # Обновляем статусы должников перед выборкой
...
#
#         # Выполнение агрегации для annual_fee
...
#         if student_class:
#             filters &= Q(student_class_id=student_class)
#         if overdue_date_gte:
...
#
#         # Выполнение агрегации для annual_fee по всей выборке студентов
...
#
#         # Получение данных по оплатам студентов для всей выборки
...
#             student.debt_amount = debt_amount
#
#         # Добавляем данные для текущей страницы студентов
#         for student in students:
#             student_id = student.id
...
#
#         # Получение данных студентов с фильтрацией
...
#         student_class = request.GET.getlist('student_class')
...
#             filters &= Q(last_name__icontains=last_name)
#         if student_class:
#             filters &= Q(student_class_id__in=student_class)
#         if group:
#             filters &= Q(group_id__in=group)
#         if class_number:
...
#
#         # Добавление строки с общей суммой годовых оплат всех учеников
#         row_num += 1
#         total_row = [
...
#             cell.value = column_title
#
#         # Получаем фильтры из представления DailyPayments
...
#         payments = TuitionFeeSplit.objects.filter(student_id__in=student_ids)
#
#         # Получаем даты для фильтрации платежей
...
#                 queryset = queryset.filter(student_class_id=filters.get('student_class'))
